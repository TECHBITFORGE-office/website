(function() {
  ...
  if (document.getElementById('APINOW-badge-wrapper')) {
    return;
  }

  // Don't show badge on huggingface.co/APINOW
  const hostname = window.location.hostname;
  const pathname = window.location.pathname;
  if (hostname === 'huggingface.co' && pathname.startsWith('/techbitforge/APINOW')) {
    return;
  }
  ...
  style.textContent = `
    ...
    #APINOW-badge-wrapper i::before {
      ...
      opacity: 0;
      background: conic-gradient(from 0deg at 50% 50%, #b4005a75, #124e00ff, #0062ffff, #ec4848ff);
      width: calc(100% * 2);
    ...
    #APINOW-badge-wrapper:hover i::before {
      opacity: 1;
      animation: APINOW-spin 3s linear infinite;
    }
  `;
  ...
  // Create text
  const text = document.createTextNode('APINOW- Webcoder🥰');
  ...
  Object.assign(badgeWrapper.style, {
    ...
    color: '#474b96ff',
    borderRadius: '9999px',
    background: 'rgba(19, 101, 160, 0.68)',
    fontSize: '12px',
    fontWeight: '500',
