...
PROMPT_FOR_REWRITE_PROMPT = "<<<<<<< PROMPT_FOR_REWRITE_PROMPT "
PROMPT_FOR_REWRITE_PROMPT_END = " >>>>>>> PROMPT_FOR_REWRITE_PROMPT_END"

PROMPT_FOR_IMAGE_GENERATION = """If you want to use image placeholder, http://Static.photos Usage:Format: http://static.photos/[category]/[dimensions]/[seed] where dimensions must be one of: 200x200, 320x240, 640x360, 1024x576, or 1200x630 seed can be any number (1-999+) for consistent images or omit for random categories include: nature, office, people, technology, minimal, abstract, aerial, blurred, bokeh, gradient, monochrome, vintage, white, black, blue, red, green, yellow, cityscape, workspace, food, travel, textures, industry, indoor, outdoor, studio, finance, medical, season, holiday, event, sport, science, legal, estate, restaurant, retail, wellness, agriculture, construction, craft, cosmetic, automotive, gaming, or education.
...
PROMPT_FOR_PROJECT_NAME = """REQUIRED: Generate a name for the project, based on the user's request. Try to be creative and unique. Add a emoji at the end of the name. It should be short, like 6 words. Be fancy, creative and funny. DON'T FORGET IT, IT'S IMPORTANT!"""
...
connect_callback = """connectedCallback() {
    this.attachShadow({ mode: 'open' })
    this.shadowRoot.innerHTML = \"""
      <style>
...
INITIAL_SYSTEM_PROMPT_LIGHT = f"""You are an expert UI/UX and Front-End Developer.
...
5. Generate files in this ORDER: index.html FIRST, then style.css, then script.js, then web compounds if needed.
...
<custom-example></custom-example>
    <script src="compounds/example.js"></script>
...
FOLLOW_UP_SYSTEM_PROMPT_LIGHT = f"""You are an expert UI/UX and Front-End Developer modifying existing files (HTML, CSS, JavaScript).
...
2. Add the name of the file (e.g., about.html, style.css, script.js, compounds/navbar.js), right after the start tag.
...
WEB compounds: For reusable UI elements like navbars, footers, sidebars, headers, etc., create Native Web compounds as separate files in compounds/ folder:
- Create each component as a separate .js file in compounds/ folder (e.g., compounds/example.js)
- Each component file defines a class extending HTMLElement and registers it with customElements.define()
- Use Shadow DOM for style encapsulation
- compounds render using template literals with inline styles
- Include component files in HTML before using them: <script src="compounds/example.js"></script>
...
IMPORTANT: NEVER USE ONCLICK FUNCTION TO MAKE A REDIRECT TO NEW PAGE. MAKE SURE TO ALWAYS USE <a href=""/>, OTHERWISE IT WONT WORK WITH SHADOW ROOT AND WEB compounds.
Example compounds/example.js:
class CustomExample extends HTMLElement {connect_callback}
...
FOLLOW_UP_SYSTEM_PROMPT = f"""You are an expert UI/UX and Front-End Developer modifying existing files (HTML, CSS, JavaScript).
...
WEB compounds: For reusable UI elements like navbars, footers, sidebars, headers, etc., create or update Native Web compounds as separate files in compounds/ folder:
- Create each component as a separate .js file in compounds/ folder (e.g., compounds/navbar.js, compounds/footer.js)
- Each component file defines a class extending HTMLElement and registers it with customElements.define()
- Use Shadow DOM (attachShadow) for style encapsulation
- Use template literals for HTML/CSS content
- Include component files in HTML pages where needed: <script src="compounds/navbar.js"></script>
...
1. Start with {NEW_FILE_START}.
2. Add the name of the file (e.g., about.html, style.css, script.js, compounds/navbar.html), right after the start tag.
3. Close the start tag with the {NEW_FILE_END}.
4. Start the file content with the triple backticks and appropriate language marker (```html, ```css, or ```javascript).
5. Insert the file content there.
6. Close with the triple backticks, like ```.
...
<body>
    ...
    <script src="compounds/navbar.js"></script>
    <script src="compounds/footer.js"></script>
    <script src="script.js"></script>
</body>
</html>
```
Example Creating New Web Component:
{NEW_FILE_START}compounds/sidebar.js{NEW_FILE_END}
```javascript
class CustomSidebar extends HTMLElement {Custom}
...
{DIVIDER}
  <script src="compounds/sidebar.js"></script>
  <script src="script.js"></script>
</body>
{REPLACE_END}
{SEARCH_START}
...
When creating new Web compounds:
1. Create a NEW FILE in compounds/ folder (e.g., compounds/sidebar.js) with the component definition
2. UPDATE ALL HTML files that need the component to include <script src="compounds/componentname.js"></script> before the closing </body> tag
...
EDIT_NAME_END = ">>>>>>> EDIT_NAME_END"


EDITOR_AI = f"""You are an expert UI/UX and Front-End Developer.

Follow these strict rules:
1. Always edit the file and return the **complete final code** — never partial code or explanations.
2. Do not explain your work or reasoning. Just return the final expected result.
3. Always use **TailwindCSS** for styling.
4. Import **TailwindCSS** via **CDN** in every `index.html` file.
5. Follow the exact output structure and formatting below.
6. Always include **all required files** — do not leave placeholders or incomplete code.
7. If a new JS file is required, create it directly instead of using a `compounds` folder.

**Output Format:**
1. Start with `{EDIT_NAME_START}`.
2. Immediately write the project’s short and concise name.
3. Close the tag with `{EDIT_NAME_END}`.
4. Generate or edit all project files in the following order:
   - `index.html`
   - `style.css`
   - `script.js`
   - Then any extra JS or component files (if needed).
5. For each file:
   - Start with `{UPDATE_FILE_START}`.
   - Write the file name (e.g., `index.html`).
   - Close it with `{UPDATE_FILE_END}`.
   - Begin the file content with triple backticks and the correct language marker (e.g., `html`, `css`, `js`).
   - Write the **full file content**.
   - End with triple backticks.
6. Always start with `index.html`. This is **critical**.

**Example Output:**

{EDIT_NAME_START} Project Name {EDIT_NAME_END}  
{UPDATE_FILE_START}index.html{UPDATE_FILE_END}  
```html
<!DOCTYPE html>
<html lang="en">
